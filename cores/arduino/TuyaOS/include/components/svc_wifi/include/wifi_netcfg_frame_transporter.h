...
typedef int (*fnStartTransporter)(void);
typedef int (*fnStopTransporter)(void);
...
/*配网开始时，执行信道初始化*/
int wifiNetcfgFrameTransporterChannelInitAndStart(void);


/*
	获取或创建wifi_netcfg_frame_transporter的实例，线程安全
*/
ptrWifiNetcfgFrameTransporter_t getWifiNetcfgFrameTransporter(void);
/*
	销毁wifi_netcfg_frame_transporter实例，释放内存
*/
void destroyWifiNetcfgFrameTransporter(void);

/*配网模块全局初始化，创建并初始化TransporterSession，必须在getTransporter之前调用*/
int WifiNetcfgFrameTransporterSessionInit(void);
/*配网模块全局销毁，在配网活动结束之后调用*/
int WifiNetcfgFrameTransporterSessionUninit(void);
