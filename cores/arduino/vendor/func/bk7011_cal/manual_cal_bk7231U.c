...
void manual_cal_save_txpwr(UINT32 rate, UINT32 channel, UINT32 pwr_gain)
{
    ...
    if((rate <= MAX_RATE_FOR_B_5M) || (rate == MAX_RATE_FOR_B)) {
        ...
        MCAL_WARN("Manual cal wrong rate:%d\r\n", rate);
        return;
    }
...
void manual_cal_show_txpwr_tab(void)
{
    ...
    MCAL_FATAL("txpwr table for b:\r\n");
    for(i=0; i<WLAN_2_4_G_CHANNEL_NUM; i++) {
        txpwr_tab_ptr = &gtxpwr_tab_b[i];
        MCAL_FATAL("ch:%2d: value:%02x, flag:%01d, gain:%02d\r\n", 
...
write_again:
...

    {
        ...
        if(check_result) 
        {
            ...
            null_prf("\r\n");
            #endif
            MCAL_FATAL("manual_cal_write_flash ok\r\n");
...
{   
    ...
    MCAL_FATAL("system temperature:%0d\r\n", g_cur_temp);
}
...
void manual_cal_load_lpf_iq_tag_flash(void)
{
    ...
    addr_start = manual_cal_search_txpwr_tab(TXID, pt->partition_start_addr);//TXPWR_TAB_FLASH_ADDR); 
    if(!addr_start) {
        MCAL_WARN("NO TXID found in flash, use lpf i&q:%d, %d\r\n",
    ...
    if(!addr)
    {
        MCAL_WARN("NO TXID_LPFCAP found in flash, use def %d, %d\r\n", 
            g_lpf_cal_i, g_lpf_cal_q);
        goto init_lpf;
    }
...
void manual_cal_load_differ_tag_from_flash(void)
{
    ...
    addr_start = manual_cal_search_txpwr_tab(TXPWR_TAB_TAB, pt->partition_start_addr);//TXPWR_TAB_FLASH_ADDR); 
    if(!addr_start) {
        MCAL_WARN("NO TXPWR_TAB_TAB found in flash, use def:%d, %d\r\n", 
    ...
    addr = manual_cal_search_txpwr_tab(TXPWR_TAB_DIF_GN20_ID, addr_start);  
    if(!addr) {
        MCAL_WARN("NO TXPWR_TAB_DIF_GN20_ID found in flash, use def:%d\r\n", MOD_DIST_G_BW_N20);
    } else {
        flash_handle = ddev_open(FLASH_DEV_NAME, &status, 0);
        ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), addr);
        ddev_read(flash_handle, (char *)&status, head.len, addr + sizeof(TXPWR_ELEM_ST));
    ...
    addr = manual_cal_search_txpwr_tab(TXPWR_TAB_DIF_GN40_ID, addr_start);  
    if(!addr) {
        MCAL_WARN("NO TXPWR_TAB_DIF_GN40_ID found in flash, use def:%d\r\n", TXPWR_TAB_DIF_GN40_ID);
    } else {
        flash_handle = ddev_open(FLASH_DEV_NAME, &status, 0);
        ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), addr);
        ddev_read(flash_handle, (char *)&status, head.len, addr + sizeof(TXPWR_ELEM_ST));
...
int manual_cal_load_xtal_tag_from_flash(void)
{
...
#if (CFG_SOC_NAME == SOC_BK7231N)
	UINT32 device_id = sctrl_ctrl(CMD_GET_DEVICE_ID, NULL);

	if ((device_id == DEVICE_ID_BK7231N_B)
	 || (device_id == DEVICE_ID_BK7231N_C))
	{
		xtal = DEFAULT_TXID_XTAL_C;
	}
#endif

    addr_start = manual_cal_search_txpwr_tab(TXID, pt->partition_start_addr);//TXPWR_TAB_FLASH_ADDR); 
    if(!addr_start) {
        MCAL_WARN("NO TXID found in flash, use def xtal:%d\r\n", DEFAULT_TXID_XTAL); 
        goto init_xtal;
    }
    ...
    if(!addr)
    {
        MCAL_WARN("NO TXID_THERMAL found in flash, use def xtal:%d\r\n", DEFAULT_TXID_XTAL);
        goto init_xtal;
    }
...
int manual_cal_load_temp_tag_from_flash(void)
{
    ...
    addr_start = manual_cal_search_txpwr_tab(TXID, pt->partition_start_addr);//TXPWR_TAB_FLASH_ADDR); 
    if(!addr_start) {
        MCAL_WARN("NO TXID found in flash, use def temp:%d\r\n", DEFAULT_TXID_THERMAL); 
        goto init_temp;
    }
    ...
    if(!addr)
    {
        MCAL_WARN("NO TXID_THERMAL found in flash, use def temp:%d\r\n", DEFAULT_TXID_THERMAL);
        goto init_temp;
    }
   ...
   /// TXPWR_ELEM_ST head;
    ...
    if(status) {
        ...
        if(!addr_start) {
            ...
            buf = (UINT8*)os_malloc(len); 
            if(!buf) {
                MCAL_WARN("no memory for save_cailmain_tx_tab\r\n");
                ddev_close(flash_handle);
                return 0;
            }
            // copy flash
            ...
            txpwr_buf = buf = (UINT8*)os_malloc(txpwr_len);
            if(!buf) {
                MCAL_WARN("no memory for txpwr tab save to flash\r\n");
                ddev_close(flash_handle);
                return 0;
            }
        }        
   ...
   /// TXPWR_ELEM_ST head;
    ...
    if(status) {
        ...
        if(!addr_start) {
            ...
            buf = (UINT8*)os_malloc(len); 
            if(!buf) {
                MCAL_WARN("no memory for save_cailmain_rx_tab\r\n");
                ddev_close(flash_handle);
                return 0;
            }
            // copy flash
    ...
    if(g_tmp_pwr.flag)
    {
        ...
        *last = g_tmp_pwr.temp_tab[g_tmp_pwr.indx];
        ...
        if (!get_ate_mode_state())
        {
            ...
            MCAL_FATAL("temp_code:%d - adc_code:%d - adc_trend:[%d]:%d->[%d]:%d\r\n",
                temp_code, cur_val,
...
void manual_cal_do_xtal_cali(UINT16 cur_val, UINT16 *last, UINT16 thre, UINT16 init_val)
{
    ...
    if((*last != param) && (foud)) {
        ...
        *last = param;
        ...
        os_null_printf("T0:%d,T1:%d,T:%d,X:%d, CT0:%d,CT:%d\r\n", g_cur_temp_flash, cur_val, thre, 
            dist, init_val, param); 
...
#if (CFG_SOC_NAME == SOC_BK7231N)
    UINT8 values[3];
    values[0] = wifi_read_efuse(18);
    values[1] = wifi_read_efuse(19);
    values[2] = wifi_read_efuse(20);
    if (((0xFF == values[0]) && (0xFF == values[1]))
     || ((0x00 == values[0]) && (0x00 == values[1]))
     || (0xFF == values[2]) || (0x00 == values[2]))
    {
        os_printf("uncali adc value:[%02x %02x %02x]\r\n", values[0], values[1], values[2]);
        goto FAILURE;
    }
    saradc_val.low = values[2] & 0x0F;
    saradc_val.low = (saradc_val.low << 8) | values[0];

    saradc_val.high = values[2] & 0xF0;
    saradc_val.high = (saradc_val.high << 4) | values[1];
#else
    UINT32 status, addr, addr_start;
    DD_HANDLE flash_handle;
    TXPWR_ELEM_ST head;
