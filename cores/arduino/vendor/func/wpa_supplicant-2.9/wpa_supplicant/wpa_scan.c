			      ...
			      size_t max_ssids, const u8 *ssid, size_t ssid_len)
{
	...
	for (j = 0; j < params->num_ssids; j++) {
		...
		if (0 == os_memcmp(params->ssids[j].bssid, ssid->bssid, sizeof(ssid->bssid)))
			break;
#endif
	}
...
#if CFG_SUPPORT_BSSID_CONNECT
	os_memcpy(params->ssids[params->num_ssids].bssid, ssid->bssid, sizeof(ssid->bssid));
#endif
	params->num_ssids++;
}
...
#if CFG_SUPPORT_BSSID_CONNECT
		os_memcpy(params->ssids[i].bssid, wpa_s->ssids_from_scan_req[i].bssid, sizeof(params->ssids[i].bssid));
#endif
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	struct wpa_ssid *ssid;
	int ret, p2p_in_prog __maybe_unused;
	struct wpabuf *extra_ie = NULL;
	struct wpa_driver_scan_params params;
	...
	if (connect_without_scan) {
		...
		if (ssid) {
			...
			bk_printf("%s: connect_without_scan set but cannot find wpa_ssid\n");
		}
	}
...
#if CFG_SUPPORT_BSSID_CONNECT
			os_memcpy(params.ssids[0].bssid, wpa_s->current_ssid->bssid, sizeof(params.ssids[0].bssid));
#endif
			params.num_ssids = 1;
		} else {
...
#if CFG_SUPPORT_BSSID_CONNECT
		os_memcpy(params.ssids[0].bssid, ssid->bssid, sizeof(params.ssids[0].bssid));
#endif
		params.num_ssids = 1;
		...
		while (ssid) {
			if (!wpas_network_disabled(wpa_s, ssid) &&
				...
				params.ssids[params.num_ssids].ssid_len =
					ssid->ssid_len;
#if CFG_SUPPORT_BSSID_CONNECT
				os_memcpy(params.ssids[params.num_ssids].bssid,
					ssid->bssid, sizeof(params.ssids[params.num_ssids].bssid));
#endif
				params.num_ssids++;
				if (params.num_ssids + 1 >= max_ssids)
					break;
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (res == 1) {
		...
		os_printf("Ignore new scan request for %d.%06d sec since an earlier "
	...
	while (ssid) {
		...
		if (params.num_filter_ssids < wpa_s->max_match_sets &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "add to filter ssid: %s",
				wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
#if CFG_SUPPORT_BSSID_CONNECT
			os_memcpy(params.filter_ssids[params.num_filter_ssids].bssid,
				  ssid->bssid, sizeof(ssid->bssid));
#endif
		       ...
		       struct wpa_scan_results *scan_res)
{
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ &&
		...
		manual_scan_use_id = wpa_s->manual_scan_id;
...
#if CFG_WPA_CTRL_IFACE
int wpa_is_scan_only()
{
	struct wpa_supplicant *wpa_s = wpa_suppliant_ctrl_get_wpas();

	if (wpa_s && wpa_s->scan_res_handler == scan_only_handler)
		return 1;
	return 0;
}
#endif
...
#if CFG_SUPPORT_BSSID_CONNECT
		if (0 == src->ssids[i].ssid_len) {
			os_memcpy(params->ssids[i].bssid, src->ssids[i].bssid,
				  sizeof(params->ssids[i].bssid));
			continue;
		}
#endif
		if (src->ssids[i].ssid) {
...
#if CFG_SUPPORT_BSSID_CONNECT
			os_memcpy(params.filter_ssids[i].bssid, ssid->bssid, sizeof(ssid->bssid));
#endif
