...
void wpa_config_free(struct wpa_config *config)
{
	...
	struct wpa_cred *cred __maybe_unused, *cprev __maybe_unused;
	int i;
...
void wpa_config_update_psk(struct wpa_ssid *ssid)
{
	...
	int count = 0, found = 0, ret;
	while (1) {
		if (!(ret = wpa_get_psk_from_cache(ssid))) {
			found = 1;
			break;
		}
		...
		// still in caculation, not available now.
		rtos_delay_milliseconds(100);
		if (count++ > 100)
			break;
	}
	if (!found) {
		wpa_printf(MSG_WARNING, "PSK caculation in wpas thread");
