			...
			int external, const u8 *sa)
{
	...
	if (auth_transaction == 1 &&
		...
		const u8 *bssid __maybe_unused = sa ? sa : wpa_s->pending_bssid;
	...
	if (auth_transaction == 1) {
		...
		if (sae_process_commit(&wpa_s->sme.sae) < 0) {
			wpa_printf(MSG_INFO, "SAE: Failed to process peer "
				   "commit");
			return -1;
		}
...
int sme_proc_obss_scan(struct wpa_supplicant *wpa_s)
{
	...
	const u8 *ie;
	u16 ht_cap;
	u8 chan_list[50/* P2P_MAX_CHANNELS*/], channel;
