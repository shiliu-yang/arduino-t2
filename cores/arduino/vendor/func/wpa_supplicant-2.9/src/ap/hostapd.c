						 ...
						 int err)
{
	...
	if (iface->conf->channel) {
		...
		if (!(iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD)) {
			/* Check DFS */
			int res = hostapd_handle_dfs(iface);
			if (res <= 0) {
				if (res < 0)
					goto fail;
				return res;
				    ...
				    const char *config_file)
{
	...
	conf = interfaces->config_read_cb(hapd_iface->config_fname);	//hostapd_config_read
	if (conf == NULL)
		goto fail;
...
int hostapd_add_iface(struct hapd_interfaces *interfaces, char *buf)
{
	...
	if (os_strncmp(buf, "bss_config=", 11) == 0) {
		...
		*pos++ = '\0';
		...
		if (new_iface) {
			if (interfaces->driver_init(hapd_iface))	//hostapd_driver_init
				goto fail;
