			   ...
			   u8 *confirm)
{
	...
	addr[0] = sc;
	len[0] = 2;
	crypto_bignum_to_bin(scalar1, scalar_b1, SAE_MAX_PRIME_LEN,
			     sae->tmp->prime_len);
	addr[1] = scalar_b1;
			       ...
			       u8 *confirm)
{
	...
	element_b1 = os_malloc(SAE_MAX_PRIME_LEN);
	element_b2 = os_malloc(SAE_MAX_PRIME_LEN);
	if (!element_b1 || !element_b2) {
		os_free(element_b1);
		os_free(element_b2);
		return;
	...
	crypto_bignum_to_bin(element1, element_b1, SAE_MAX_PRIME_LEN,
			     sae->tmp->prime_len);
	crypto_bignum_to_bin(element2, element_b2, SAE_MAX_PRIME_LEN,
			     sae->tmp->prime_len);
